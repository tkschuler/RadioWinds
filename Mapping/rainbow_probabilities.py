...
method = 'linear'
year = 2012
prefix = "North_America"  #title of the maps that are exported to the MAPS folder
...
for month in range (1,12+1):
    ...
    ax.set_title(prefix + " " + config.mode + "_" + type+ "_" + "\n Opposing Winds Probabilities\n Alt:{15-25 km} in " + calendar.month_name[month] + " " + str(year), fontsize=24)
    plt.tight_layout()

    print("generating map for " + prefix + "_" + type+ "_" + config.mode +  "-" + str(year) + '-' + str(month))
    ...
    plt.savefig(path +"/" +  prefix + "_" + type+ "_" + config.mode + "-" + str(year) + '-' + str(month))
    #plt.show()
