...
def determine_opposing_winds(df , wind_bins, n_sectors, speed_threshold = 4):
    ...
    for i in range (0,int(n_sectors/2)):
        #print(i, np.sum(table, axis=0)[i], np.sum(table, axis=0)[i+int(n_sectors/2)])
        #check if opposing sectors in the histogram tables have values greater than 0 (therefore, there are winds in that sectors)
