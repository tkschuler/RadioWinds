...
def save_wind_probabilties(FAA, WMO, wind_probabilities, analysis_folder, date):
    ...
    print(colored(
        "Processing Full Winds data for Station-" + str(FAA) + " - " + str(WMO) +
    ...
    utils.export_colored_dataframes(wind_probabilities,
                                    title='Full Wind Probabilities for Station ' + str(FAA) + " - " + str(WMO) +
                                          ' in Month ' + str(date.month) + ' - ' + str(date.year),
                                    path=analysis_folder,
                                    suffix=str(FAA) + " - " + str(WMO) + "-" + str(date.year) + "-" + str(date.month) + "-FULL",
...
def analyze_annual_data(FAA, WMO, year, min_alt=15000, max_alt=28000,
    ...
    utils.export_colored_dataframes(annual_probabilities,
                                    title='Full Wind Probabilities for Station ' + str(FAA) + " - " + str(
                                        WMO) + ' in ' + str(year),
                                    path=config.analysis_folder + str(FAA) + " - " + str(WMO) + "/",
                                    suffix="analysis_" + str(year) + '-wind_probabilities-FULL',
...
def batch_analysis(era5, year, WMO, FAA, lat, lon, min_alt, max_alt,
    ...
    """
    if config.mode == "era5":
        print(colored("ERA5 forecasts not supported for full winds batchy analysis", "red"))
        sys.exit()
